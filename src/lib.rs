...
fn impl_handler_macro(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let trait_name_exact_with_prefix =
        syn::Ident::new(&format!("IterToDataFrame{name}WithPrefix"), name.span());
    let trait_name_dyn = syn::Ident::new(&format!("IterToDataFrameDyn{name}"), name.span());

    let doc_msg_exact = format!(
        "Trait to convert an exact size iterator with elements of type {name} into a polars DataFrame.",
    );

    let doc_msg_dyn = format!(
        "Trait to convert an iterator with elements of type {name} into a polars DataFrame.",
    );
    ...
    let gen = quote! {
        ...
        #[automatically_derived]
        #public trait #trait_name_exact_with_prefix {
            #[doc = #doc_msg_exact]
            fn to_dataframe_with_prefix(self, prefix: String) -> Result<polars::frame::DataFrame, polars::prelude::PolarsError>;
        }
        ...
        #[automatically_derived]
        impl<T> #trait_name_exact_with_prefix for T
        where
            T: Iterator<Item = #name> + ExactSizeIterator,
        {
            fn to_dataframe_with_prefix(self, prefix: String) -> Result<polars::frame::DataFrame, polars::prelude::PolarsError> {
                ...
                polars::frame::DataFrame::new(
                    vec![
                        #(polars::frame::column::Column::new(
                            format!("{prefix}_{}", stringify!(#named_fields)).into(),
                            #named_fields
                        ),)*
                    ]
                )
